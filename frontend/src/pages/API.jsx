...
const Introduction = () => {
  const keyFeatures = [
    {
      icon: (
        <svg fill="currentColor" viewBox="0 0 256 256" xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 sm:w-7 sm:h-7">
        ...
        <main className="flex-1 lg:ml-0 min-h-screen p-4 sm:p-6 lg:p-8 xl:p-10 overflow-auto">
          <div className="max-w-4xl mx-auto w-full">
            ...
            <section className="mb-6 sm:mb-8 lg:mb-10 xl:mb-12">
              <h2 className="text-xl font-bold text-white mb-3 sm:mb-4 sm:text-2xl lg:text-3xl lg:mb-6">Key Features</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 lg:gap-6">
                {keyFeatures.map((feature, index) => (
                  <div 
                    key={index} 
                    className="flex items-start gap-3 p-3 sm:p-4 rounded-lg bg-background-dark/50 border border-primary/20 hover:border-primary/30 transition-colors duration-200"
                  >
                    <div className="flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 flex items-center justify-center rounded-lg bg-primary/20 text-primary">
                      {feature.icon}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-white mb-1 text-sm sm:text-base lg:text-lg truncate">{feature.title}</h3>
            ...
            {/* Use Cases Section */}
            <section className="mb-6 sm:mb-8 lg:mb-10 xl:mb-12">
              <h2 className="text-xl font-bold text-white mb-3 sm:mb-4 sm:text-2xl lg:text-3xl lg:mb-6">Use Cases</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 lg:gap-6">
                {useCases.map((useCase, index) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-bold text-white mb-1 text-sm sm:text-base lg:text-lg truncate">{useCase.title}</h3>
                      <p className="text-xs sm:text-sm text-gray-300 leading-relaxed">{useCase.description}</p>
                    </div>
                  </div>
                ))}
              </div>
